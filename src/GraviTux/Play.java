...
public class Play extends BasicGameState {
    ...
    private Animation tux, movingUp, movingDown, movingLeft, movingRight, standing, standing90; //walk animations
    private Image[] walkLeft, walkRight, walkUp, walkDown, stand, stand90;   //Image arrays used for animations
    private TiledMap worldMap;  //Level in the background
    private boolean menu, revGravi, sGravi;   //states if menu is open and if gravity is reversed
    ...
    public Play(int state) {
        ...
        sGravi = false; //true, if 90° storm flip
    ...
    public void init(GameContainer gc, StateBasedGame sbg) throws SlickException {
        ...
        walkUp = new Image[]{new Image("GraviTux/tux/Tux_up01.png"), new Image("GraviTux/tux/Tux_up02.png"), new Image("GraviTux/tux/Tux_up03.png"), new Image("GraviTux/tux/Tux_up04.png")}; //these are the images to be used in the "walkUp" animation
        walkDown = new Image[]{new Image("GraviTux/tux/Tux_down01.png"), new Image("GraviTux/tux/Tux_down02.png"), new Image("GraviTux/tux/Tux_down03.png"), new Image("GraviTux/tux/Tux_down04.png")};
        ...
        stand90 = new Image[]{new Image("GraviTux/tux/tux_standing90.png")};
        ...
        standing90 = new Animation(stand90, duration, false);
    ...
    public void update(GameContainer gc, StateBasedGame sbg, int delta) throws SlickException {
        ...
        ////move left
        if ((input.isKeyDown(Input.KEY_LEFT) || input.isKeyDown(Input.KEY_A)) && (!sGravi)) {
        ...
        ////move right
        if ((input.isKeyDown(Input.KEY_RIGHT) || input.isKeyDown(Input.KEY_D)) && (!sGravi)) {
            tux = movingRight;
        ...
        ////flip Gravity, but only when touching the ground and no 90°Gravi set
        ...
        if ((sGravi) && (input.isKeyDown(Input.KEY_UP) || input.isKeyDown(Input.KEY_W))) {
            tux = movingUp;
            ...
            if ((input.isKeyDown(Input.KEY_DOWN) || input.isKeyDown(Input.KEY_S)) && (sGravi)) {
                tux = movingDown;
        ...
        */
        ...
        ////Gravity
        if ((!(isBlocked(tuxX, tuxY + size + (vSpeed + gravity * delta)) || isBlocked(tuxX + size - 1, tuxY + size + (vSpeed + gravity * delta)))) && (!revGravi) && (!sGravi)) {
            ////accelerate falling
        ...
        ////negative gravity
        else if ((!(isBlocked(tuxX, tuxY - (vSpeed + gravity * delta)) || isBlocked(tuxX + size - 1, tuxY - (vSpeed + gravity * delta)))) && (revGravi) && (!sGravi)) {
            ////accelerate falling
            vSpeed += gravity * delta;
            ////limit falling speed
            if (vSpeed > vSpeedMax) {
        ...
        ////negative gravity 90°
        else if ((!(isBlocked(tuxX, tuxY - (vSpeed + gravity * delta)) || isBlocked(tuxX + size - 1, tuxY - (vSpeed + gravity * delta)))) && (revGravi) && (sGravi))//fuu
        {
            ////accelerate falling
            vSpeed += gravity * delta;
            ////limit falling speed
