...
//    private static Audio test;
    ...
    public void init(GameContainer gc, StateBasedGame sbg) throws SlickException {
        /*try
        {
            test = AudioLoader.getAudio("OGG", ResourceLoader.getResourceAsStream("res/GraviTux/sounds/gravitation.ogg"));
        }
        catch (IOException e)
        {
            e.printStackTrace();
        ...
        //sounds, for some reason ogg do not work correctly....        /*storms = new Sound("res/GraviTux/sounds/eissturm.wav");
...
/*		g.drawString("Textmasse als Hilfe zum Platzieren von Text", 40, 70);
        ...
        ////level done event
        if (inputDelay.isTimeElapsed() && (collision(tuxX + 9, tuxY + 10, levelEnd)
                || collision(tuxX + tuxWidth - 9, tuxY + 10, levelEnd)
                || collision(tuxX + 9, tuxY + tuxHeight - 10, levelEnd)
                || collision(tuxX + tuxWidth - 9, tuxY + tuxHeight - 10, levelEnd))) {
...
//                AL.destroy();
                ...
                buildCollisionMap();    //builds the collisionmap fur the current
                tuxReset();
            }
            inputDelay.reset();
        }
...
//              /**/gravitation.play();
//                test.playAsSoundEffect(1.0f, 1.0f, false);
//                SoundStore.get().poll(0);
    ...
    //builds the collitionmap of the current level
    public static void buildCollisionMap() {
        //build collision maps based on tile properties in the Tiled map                                                                  <
        blocked = new boolean[20][15];
        deadly = new boolean[20][15];
        levelEnd = new boolean[20][15];
        storm = new boolean[20][15];
